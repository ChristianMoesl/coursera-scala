...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = 
    rdd.aggregate(0)((res, article) => if (article.mentionsLanguage(lang)) res + 1 else res, (_ + _))
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = 
    index.map(x => (x._1, x._2.count(a => true)))
         .collect
         .toList
         .sortBy(_._2)(Ordering[Int].reverse)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = 
    rdd.flatMap(article => 
      langs.filter(article.mentionsLanguage(_)).map((_, 1))
    ).reduceByKey(_ + _)
     .collect
     .toList
     .sortBy(_._2)(Ordering[Int].reverse)
