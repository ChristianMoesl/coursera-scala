...
object TimeUsage extends TimeUsageInterface {
  ...
  /** @return The initial data frame columns partitioned in three groups: primary needs (sleeping, eating, etc.),
    ...
    */
  val primaryNeedsStart = Set("t01", "t03", "t11", "t1801", "t1803")
  val workingActivitiesStart = Set("t05", "t1805")
  val otherActivitesStart = Set("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    columnNames match {
      case Nil => (Nil, Nil, Nil)
      case head::tails =>
        val other = classifiedColumns(tails)
        head match {
          case s if primaryNeedsStart.exists(s.startsWith(_)) => (new Column(s)::other._1, other._2, other._3)
          case s if workingActivitiesStart.exists(s.startsWith(_)) => (other._1, new Column(s)::other._2, other._3)
          case s if otherActivitesStart.exists(s.startsWith(_)) => (other._1, other._2, new Column(s)::other._3)
          case _ => other
      }
    }
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round(v: Double) = math.round(v * 10.0D) / 10.0D
    ...
    summed.groupByKey(r => (r.working, r.sex, r.age))
          ...
          .map{ case ((k1, k2, k3), v1, v2, v3) => 
            TimeUsageRow(k1, k2, k3, round(v1), round(v2), round(v3)) }
          .sort("working", "sex", "age")
  }
}
