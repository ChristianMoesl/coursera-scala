...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] =
      chars.groupBy(f => f).map(x => (x._1, x._2.length)).toList
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case Nil => trees
      case ::(head, tl) => if (singleton(trees)) trees
        else {
        val l = head
        val r = tl.head
        makeCodeTree(l, r)::tl.tail
      }
    }
   ...
   */
    def convert(tree: CodeTree): CodeTable = tree match {
      case Fork(left, right, _, _) => convert(left).map(x => (x._1, 0::x._2)):::convert(right).map(x => (x._1, 1::x._2))
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      val t = convert(tree)
      text.flatMap(c => t.filter(p => p._1 == c).head._2)
    }
  }
