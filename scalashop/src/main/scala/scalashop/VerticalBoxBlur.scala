...
object VerticalBoxBlur {
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    val steps = Math.ceil(src.width.toDouble / numTasks.toDouble).toInt
    val seperators = 0 until src.width by steps
    val seperatorsWithLast = if (seperators.contains(src.width)) seperators else seperators.toList:::List(src.width)
